...
@Component({
  selector: 'imdb-factory',
  templateUrl: './imdb-factory.component.html',
  styleUrls: ['./imdb-factory.component.css']
})
export class ImdbFactoryComponent implements OnInit {
  ...
  async open(content: any) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then(async (result) => {
      ...
      this.networkService.setWidgets({ data: { type: "imdb", title: this.titleFormControl.value }, cookie: user, update: 300000 }).then((data) => {
        console.log(data);
